...
class vpc:
    ...
    def create_basic_networking(self):
        vpc_pulumi = aws.ec2.Vpc(
            ...
            tags={"Name": "vpc_{}".format(self.name), "Createdby": "Pulumi"},
        )
        ...
        for i in range(len(self.cidr_public)):
            self.public_subnets.append(
                aws.ec2.Subnet(
                    "subnet_public_{}_{}".format(self.name, i),
        ...
        for i in range(len(self.cidr_private)):
            self.private_subnets.append(
                aws.ec2.Subnet(
                    "subnet_private_{}_{}".format(self.name, i),
        ...
        eip = aws.ec2.Eip(
            "eip_{}".format(self.name),
            vpc=True,
            __opts__=pulumi.ResourceOptions(provider=self.provider),
            tags={"Name": "eip_{}".format(self.name), "Createdby": "Pulumi"},
        )

        natgw = aws.ec2.NatGateway(
            "natgw_{}".format(self.name),
            allocation_id=eip.id,
            subnet_id=self.public_subnets[0].id,
            __opts__=pulumi.ResourceOptions(provider=self.provider),
            tags={"Name": "natgw_{}".format(self.name), "Createdby": "Pulumi"},
        )

        # Internet Gateway creation
        igw = aws.ec2.InternetGateway(
            "igw_{}".format(self.name),
            vpc_id=vpc_pulumi.id,
            __opts__=pulumi.ResourceOptions(provider=self.provider),
            tags={"Name": "igw_{}".format(self.name), "Createdby": "Pulumi"},
        )

        # Route Tables Creation
        # Public RT
        rt_public = aws.ec2.RouteTable(
            "rt_public_{}".format(self.name),
            routes=[{"cidr_block": "0.0.0.0/0", "gateway_id": igw.id,}],
            __opts__=pulumi.ResourceOptions(provider=self.provider),
            vpc_id=vpc_pulumi.id,
            tags={"Name": "rt_public_{}".format(self.name), "Createdby": "Pulumi"},
        )

        # Private RT
        rt_private = aws.ec2.RouteTable(
            "rt_private_{}".format(self.name),
            routes=[{"cidr_block": "0.0.0.0/0", "gateway_id": natgw.id,}],
            __opts__=pulumi.ResourceOptions(provider=self.provider),
            vpc_id=vpc_pulumi.id,
            tags={"Name": "rt_private_{}".format(self.name), "Createdby": "Pulumi"},
        )

        # Route association
        # public rt
        for i in range(len(self.public_subnets)):
            rt_association_pub = aws.ec2.RouteTableAssociation(
                "rt_association_pub_{}".format(i),
        ...
        for i in range(len(self.private_subnets)):
            rt_association_priv = aws.ec2.RouteTableAssociation(
                "rt_association_priv_{}".format(i),

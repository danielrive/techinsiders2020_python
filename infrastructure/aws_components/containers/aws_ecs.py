...
class aws_ecs:
    def __init__(self, name, provider):
        ...
        self.task_def = []
        self.cluster_info = aws.ecs.Cluster(
            "cluster_" + name,
            __opts__=pulumi.ResourceOptions(provider=provider),
            tags={"Name": "cluster_" + name, "Createdby": "Pulumi"},
        )

    def create_taskdf(self, cpu, memory, role_task=None, execution_role=None):
        container_definition = open(
            "./aws_components/containers/containers_micro.json", "r"
        )
    ...
    def create_service(
        self, name_service, min_task, max_tasks, target_groups, subnets, sg
    ):
        """
        Parameters
        ------------
        ...
        for i in range(len(target_groups)):
            mapping_alb.append(
                {
                    "target_group_arn": target_groups[i][0],
        ...
        ecs_service = aws.ecs.Service(
            "ecs_service_" + name_service + self.name,
            ...
            network_configuration={"subnets": subnets, "security_groups": [sg]},
            __opts__=pulumi.ResourceOptions(provider=self.provider),
        )
    ...
    def create_ecr_repo(self, name, sec_scaning, tags_type):
        ...
        repo = aws.ecr.Repository(
            name,
            name=name,
            image_scanning_configuration={"scanOnPush": sec_scaning,},

...
aws_config = aws.Provider(
    "aws", region=os.getenv("region"), profile=os.getenv("profile")
)
...
if __name__ == "__main__":
    ...
    vpc_microservice = aws_vpc.vpc(
        "danielr", "10.11.0.0/16", cidr_public, cidr_private, aws_config
    )
    ...
    alb_sg = security_group.create_sg(
        "sg_alb", net_info["vpc_id"], rule_alb_ingress, rule_alb_egress
    )
    ...
    create_alb = alb.create_alb(
        "danielr", net_info["private_subnets"], True, [alb_sg["sg_id"]]
    )
    ...
    task_micro = ecs_aws.create_taskdf(
        "512", "1024", task_role["role_arn"], execution_role["role_arn"]
    )
